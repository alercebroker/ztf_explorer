<template>
    ...
    <div v-else-if="$store.state.search.query_status === 200">
        <div class="row">
            <b-col cols="4">
                <b-btn
                    class="mb-3 btn-wrap-text"
                    :block="block"
                    v-b-modal.showDetails
                >Columns to show in table</b-btn>
            </b-col>
            <b-col></b-col>
        </div>
        ...
        <div
            v-if="$store.state.results.objects.length || Object.keys($store.state.results.objects).length > 0"
        >
            <b-table
                striped
                hover
            ...
            >
                ...
                <template slot="radec" slot-scope="data">
                    <div
                        v-if="data.item.meanra != null && data.item.meandec != null"
                    >({{data.item.meanra.toFixed(3) +", "+data.item.meandec.toFixed(3)}})</div>
                </template>
            </b-table>
            <b-pagination
                size="md"
                :total-rows="$store.state.results.total"
                v-model="currentPage"
